...
def return_pooling_objective_function(vr_control_func_dict:dict)->Callable[[int,SimulationVehicle,VehiclePlan,Dict[Any,PlanRequest],NetworkBase],float]:
    ...
    if func_key == "total_distance":
        ...
        def control_f(simulation_time:float, veh_obj:SimulationVehicle, veh_plan:VehiclePlan, rq_dict:Dict[Any,PlanRequest], routing_engine:NetworkBase)->float:
            ...
            sum_user_times = 0
            for rid, boarding_info_list in veh_plan.pax_info.items():
                rq_obj = rq_dict.get(rid)
                if rq_obj is None:
                    # request not present anymore -> ignore it in objective evaluation
                    continue
                rq_time = rq_obj.rq_time
                # boarding_info_list should contain [pickup_time, dropoff_time]
                if boarding_info_list is None or len(boarding_info_list) < 2:
                     # fallback: assume dropoff time equals pickup time (or current sim time if even pickup is missing)
                     pickup_time = boarding_info_list[0] if boarding_info_list and len(boarding_info_list) >= 1 else simulation_time
                     drop_off_time = pickup_time
                     # optional: log once (comment out if too noisy)
                     # LOG.warning(f"boarding_info_list incomplete for rid {rid}: {boarding_info_list}. Using fallback times.")
                else:
                    drop_off_time = boarding_info_list[1]
                sum_user_times += (drop_off_time - rq_time)

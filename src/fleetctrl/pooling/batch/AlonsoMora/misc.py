...
def deleteRidFromRtv(rid : Any, rtv_key : tuple) -> tuple:
    ...
    if rid in list_rids:
        ...
        # rid already not present -> database likely already updated; ignore to keep sim running
        # optional debug:
        # LOG.warning(f"deleteRidFromRtv: rid {rid} not in list_rids for key {previous_rtv_key}")
        pass
    if list_rids:

...
class AlonsoMoraAssignment(BatchAssignmentAlgorithmBase):
    ...
    def compute_new_vehicle_assignments(self, sim_time : int, vid_to_list_passed_VRLs : Dict[int, List[VehicleRouteLeg]], 
       ...
       # # LOG.debug(f"check for untracked boardings")
        ...
        # --- SAFETY INIT: make sure every vehicle has a key in external_assignments ---
        if self.external_assignments is None:
           self.external_assignments = {}

        # AlonsoMoraAssignment stores vehicles in self.veh_objs
        for vid in self.veh_objs.keys():
          self.external_assignments.setdefault(vid, (None, None))
        # ---------------------------------------------------------------------------
    ...
    def _getRR(self, rid1, rid2):
        ...
        if comp == 1:
            ...
            rq1, rq2 = self.active_requests[rid1], self.active_requests[rid2]
    ...
    def _checkForNecessaryV2RBsAndComputeMissing(self, vid : int):
        ...
        if assigned_v2rb is None:
            ...
            ext = self.external_assignments.get(vid, (None, None))
            assigned_plan = ext[1]
            if assigned_plan is None:
                # no assigned plan known for this vid yet -> nothing to check
                return
            # try:

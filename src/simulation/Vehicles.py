            ...
            if not list_route_legs or list_route_legs[0] != self.assigned_route[0]:
                if list_route_legs and self.status == VRL_STATES.WAITING and list_route_legs[0].earliest_start_time > sim_time: # dont write multiple waiting legs
                    ...
                    if not self.assigned_route[0].locked:
                        ...
                        # PATCH: if force_ignore_lock is True, keep locked VRL as first element and continue
                        if force_ignore_lock:
                            # keep current locked VRL first, then append new legs (avoid duplicates)
                            ...
                            if list_route_legs:
                                LOG.error("new additional infos: {}".format(list_route_legs[0].additional_str_infos()))
                                 ...
                                 LOG.error("new additional infos: <EMPTY list_route_legs>")
                    
                            LOG.warning(
                                f"Vehicle {self.vid}: locked VRL prevents reassignment at time {sim_time}. "
                                f"Ignoring new list_route_legs update."
                            )
                            return
            else:
                start_flag = False

    ...
    def assign_vehicle_plan(self, veh_obj : SimulationVehicle, vehicle_plan : VehiclePlan, sim_time : int,
        ...
        for rid in get_assigned_rids_from_vehplan(vehicle_plan):
            ...
            if rid not in self.rq_dict:
                LOG.warning(f"rid {rid} not found in rq_dict when assigning to vehicle {veh_obj.vid}")
                continue

            # pax_info should contain [pickup_time, dropoff_time]
            if pax_info is None or len(pax_info) < 2:
                LOG.warning(
                    f"Missing pax_info for rid {rid} (vehicle {veh_obj.vid}). "
                    f"pax_info={pax_info}. Using simulation time placeholders."
                )
                self.rq_dict[rid].set_assigned(sim_time, sim_time)
            else:
